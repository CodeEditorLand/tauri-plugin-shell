...
/// `None`.
///
/// The path will be matched against the shell open validation regex, defaulting
/// to `^((mailto:\w+)|(tel:\w+)|(https?://\w+)).+`. A custom validation regex
/// may be supplied in the config in `plugins > shell > scope > open`.
///
/// # Examples
///
/// ```rust,no_run
