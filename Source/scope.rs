...
use regex::Regex;
use tauri::{ipc::ScopeObject, Manager};

use crate::{open::Program, process::Command};
...
impl From<String> for ExecuteArgs {
	fn from(string:String) -> Self { Self::Single(string) }
}

impl From<Vec<String>> for ExecuteArgs {
	fn from(vec:Vec<String>) -> Self { Self::List(vec) }
}
...
impl ScopeObject for ScopeAllowedCommand {
	...
	fn deserialize<R:tauri::Runtime>(
		...
		Ok(Self { name:scope.name, command, args, sidecar:scope.sidecar })
	}
}
...
pub enum ScopeAllowedArg {
	...
	/// An argument with a value to be evaluated at runtime, must pass a regex
	/// validation.
	Var {
		/// The validation that the variable value must pass in order to be
		/// called.
		validator:Regex,
	},
}
...
impl ScopeAllowedArg {
	...
	pub fn is_fixed(&self) -> bool { matches!(self, Self::Fixed(_)) }
}
...
pub enum Error {
	...
	///
	/// This can happen from passing a string or array of strings to a command
	/// that is expecting named variables, and vice-versa.
...
impl OpenScope {
	...
	///
	/// The path is validated against the `plugins > shell > open` validation
	/// regex, which defaults to `^((mailto:\w+)|(tel:\w+)|(https?://\w+)).+`.
	pub fn open(&self, path:&str, with:Option<Program>) -> Result<(), Error> {
		...
		// The prevention of argument escaping is handled by the usage of
		// std::process::Command::arg by the `open` dependency. This behavior
		// should be re-confirmed during upgrades of `open`.
