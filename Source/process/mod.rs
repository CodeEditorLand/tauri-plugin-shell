...
pub enum CommandEvent {
	...
	/// stdout/stderr bytes to a UTF-8 string.
	Error(String),
...
impl CommandChild {
	...
	pub fn pid(&self) -> u32 { self.inner.id() }
}
...
impl From<Command> for StdCommand {
	fn from(cmd:Command) -> StdCommand { cmd.cmd }
}
...
impl Command {
	...
	/// Configures the reader to output bytes from the child process exactly as
	/// received
	...
	/// tauri::Builder::default().setup(|app| {
	/// 	let status = tauri::async_runtime::block_on(async move {
	/// 		app.shell().command("which").args(["ls"]).status().await.unwrap()
	/// 	});
